...
def build_starter_pitch_count_sentence(p: dict, label: str, seed: int) -> str:
    ...
    if strikes > 0 and bb >= 4 and pitches >= 70:
        choices.append(f"He still threw {strikes} of {pitches} pitches for strikes, but the misses that mattered stretched too many innings out.")
...
def build_starter_subject_line(p: dict, label: str, game_context: dict, seed: int) -> str:
    ...
    if outs < 3:
        choices = [
            f"{name} was knocked out before he could escape the first",
            f"{name} never made it out of the opening inning",
        ]
    elif label in {"GEM", "DOMINANT"}:
        choices = [
            f"{name} controlled this start from the jump",
            f"{name} carried the game for his club",
        ]
        if k >= 8:
            choices.append(f"{name} piled up strikeouts while working deep into the game")
    elif label in {"QUALITY", "SHARP", "SOLID"}:
        choices = [
            f"{name} gave his club a steady start",
            f"{name} kept the game in a good place through {ip_text}",
        ]
        if k >= 7:
            choices.append(f"{name} paired length with {number_word(k)} strikeouts")
    else:
        choices = [
            f"{name} never found a clean lane in this one",
            f"{name} could not keep this outing under control",
        ]
        if hits >= 7 and bb <= 1:
            choices.append(f"{name} got hit hard even without much extra traffic from walks")
        elif bb >= 4:
            choices.append(f"{name} kept falling behind and never really recovered")
        elif er >= 4:
            choices.append(f"{name} could not keep the damage from stacking up")

    if team_runs > opp_runs and label in GOOD_STARTER_LABELS | {"SOLID"}:
        choices.append(f"{name} set up a winning night for his club")
    elif team_runs < opp_runs and label in BAD_STARTER_LABELS | {"UNEVEN"}:
        choices.append(f"{name} put his club in chase mode early")
